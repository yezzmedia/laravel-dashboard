...
<header class="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-gray-200/80 bg-white/80 px-4 shadow-sm backdrop-blur-md dark:border-gray-800/80 dark:bg-gray-900/80 sm:px-6">
    {{-- Mobile menu button --}}
    <button type="button" @click="mobileMenuOpen = !mobileMenuOpen" aria-label="{{ __('dashboard::dashboard.nav.toggle_mobile') }}" class="rounded-lg p-2 text-gray-500 transition hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 lg:hidden">
        <x-filament::icon icon="heroicon-c-bars-3" class="h-5 w-5" />
    </button>

    {{-- Desktop sidebar toggle --}}
    <button type="button" @click="sidebarOpen = !sidebarOpen" aria-label="{{ __('dashboard::dashboard.nav.toggle_sidebar') }}" class="hidden rounded-lg p-2 text-gray-500 transition hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 lg:block">
        <x-filament::icon icon="heroicon-c-bars-3" class="h-5 w-5" />
    </button>
    ...
    <a href="{{ route('filament.dashboard.home') }}" class="flex items-center gap-2.5">
        <div
            ...
            style="background: {{ config('dashboard.brand.gradient') }};"
        >
        ...
        <div class="hidden flex-col leading-tight sm:flex">
            ...
            <span class="text-xs text-gray-500 dark:text-gray-400">{{ __('dashboard::dashboard.nav.control_center') }}</span>
        </div>
    </a>
    ...
    <div class="flex items-center gap-1.5">
        ...
        <button
            type="button"
            @click="toggleTheme()"
            aria-label="{{ __('dashboard::dashboard.nav.toggle_theme') }}"
            ...
            <div x-data="{
                ...
                >
                    <div
                        ...
                        style="background: {{ config('dashboard.brand.avatar_gradient') }}; color: #ffffff; font-size: 0.875rem;"
                    >
                ...
                >
                    <div class="px-4 py-3">
                        <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('dashboard::dashboard.user.signed_in_as') }}</p>
                    ...
                    <div class="p-1.5">
                        <form method="POST" action="{{ route('logout') }}" x-ref="logoutForm">
                            ...
                            >
                                ...
                                {{ __('dashboard::dashboard.user.log_out') }}
                            </button>
                        </form>
                    </div>
                </div>
